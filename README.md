...
- 멘토링 ✔️  
  - 내 지도 교수 함게 만나기   
    ❓ SQLD 자격증 준비를 위해 어떤 수업이나 내용이 도움이 될까요?
    
    ✔ 소프트웨어융합대학 교과과정 안에 SQLD와 직접적으로 연관된 ‘데이터베이스’ 과목이 포함되어 있습니다. 해당 수업에서 SQL을 실습하며 배우기 때문에 자격증 준비에 큰      도움이 됩니다. 추후에는 SQL 전용 라이브 코딩 테스트를 구성할 수 있어 학습 기회가 더 확대될 가능성도 있습니다.

    ❓ 프로그래밍 실력은 어떻게 쌓아야 할까요?
    
    ✔ 단순히 '프로그래밍을 잘하는 것'보다 ‘컴퓨터 언어를 잘 다루는 능력’이 더 중요합니다. AI가 상용화된 시대에서 AI와 의사소통하며 주도적으로 코딩할 수 있는 능력이 
    핵심입니다.즉, AI에 의존하지 말고 잘 활용해야 하며,스스로 배우려는 의지와 끝까지 질문하여 해결하는 습관이 진짜 실력을 만들어 줍니다. 배움에 대한 
    호기심과 경험을 넓히는 노력은 결국 본인의 기반을 강화하는 과정입니다.
    
    ❓ 여러 트랙을 동시에 진행해도 괜찮나요?
    
    ✔ 전혀 문제 없습니다. 트랙 제도에 딱히 얽매이지 않고 여러 트랙에 구성되어 있는 과목을 혼합하여 배우는 것도 가능합니다. 트랙을 정확히 따르는 것또 좋지만, 폭넓게 
    다양한 분야를 배우며 실질적인 능력을 쌓는 것이 훨씬 중요합니다. 나와 다른 사람 모두에게 ‘어려운 무언가’를 성취하는 경험이 큰 가치가 있으며, 호기심이 생기면 일단 
    도전해보는 자세가 필요합니다.
    
    
    ❓ 현장실습은 어떤가요? 장점이나 한계가 있을까요?
    
    ✔ 현재 현장실습 시스템은 여러모로 어려움이 있고 구조적 한계가 존재합니다. 그래서 소프트웨어융합대학 내부의 프로그램을 최대한 활용하는 것이 더 실질적 도움이 될 수 
    있습니다. 결국 중요한 것은 어디를 가느냐보다 무엇을 만들고 어떤 아이디어를 실현하느냐입니다.



  - 대학원 방문 및 선배 만나기   
    ❓ 프로그래밍 실력은 어떻게 키우는 게 좋을까요?
    
    ✔ 백준을 꾸준히 풀어보는 것이 좋습니다.
    기초 알고리즘 문제를 반복적으로 풀면서 문제 해결 능력을 기르는 것이 가장 확실한 방법입니다.
    처음엔 어려워도, 일정량을 넘기면 실력이 정말 눈에 띄게 늘어납니다.
    
    ❓ 전공 지식은 어떤 것부터 공부해야 하나요?
    
    ✔ CS 기초 지식을 지속적으로 쌓아야 합니다. 개인적으로는 선형대수가 정말 중요했습니다.
    AI·그래픽스·데이터 분야 모두 선형대수를 기반으로 하므로, 기초를 잘 다져두면 상위 수업이 훨씬 쉽습니다.
    
    ❓ 랩실에서 일할 때 처우나 환경은 어떤가요?
    
    ✔ 생각보다 괜찮습니다.
    기본 제공 PC, 개발 서버 등 장비가 잘 갖춰져 있고, 워라밸도 생각보다 나쁘지 않습니다.
    
    ❓ 외부 활동이나 네트워킹 기회를 얻기 쉽나요?
    
    ✔ 예전보다 훨씬 많습니다.
    학회 출신들이 많아서 외부 세미나·컨퍼런스·행사에 초대받을 기회도 늘었습니다.
    학생일 때부터 참여하면 인맥도 많이 쌓여요.
    
    ❓ 대학원 진학 시 학부 성적이 중요한가요?
    
    ✔ 크게 영향을 주지 않습니다.
    성적보다 연구 경험, 관심 분야, 포트폴리오, 프로젝트 경험이 더 중요합니다.
    본인이 어떤 분야를 하고 싶은지 명확히 찾는 게 우선이에요.
    
    ❓ 학부 때 무엇을 하면 도움이 될까요?
    
    ✔ 소융대 행사나 프로젝트 활동에 적극적으로 참여하세요.
    팀 프로젝트 경험은 대학원/취업 모두에서 핵심 평가 요소입니다.
    
    ❓ 정보를 어디서 찾아야 하나요?
    
    ✔ 스스로 자료를 찾아보는 습관이 정말 중요합니다.
    진로, 학회, 프로젝트, 연구 정보 등은 본인이 주도적으로 탐색해야 경쟁력이 생깁니다.
    
    ❓ 교수님과 상담할 때 어떻게 질문하면 좋을까요?
    
    ✔ 구체적인 관심 분야를 언급하며 조언을 구하면 됩니다.
    교수님께서 방향성을 잡아주실 것입니다.
    
    ❓ 1학년에게 하고 싶은 조언?
    
    ✔ 많이 노세요.
    다양한 경험을 느껴보면 장기적으로 더 큰 도움이 됩니다.
    
    ❓ 교양 과목은 어떻게 선택해야 하나요?
    
    ✔ 자신에게 맞는, 흥미 있는 교양을 선택하세요.
    이 과목들이 오히려 아이디어나 진로에 힌트를 주기도 합니다.
    억지로 버티는 교양 수업보다는 본인에게 맞는 수업이 더 효과적입니다.
